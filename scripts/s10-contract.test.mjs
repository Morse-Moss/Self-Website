...
test('S10 has one active blueprint override and an eight-stage task center', () => {
  ...
  const runPointer = runState.match(/^\*\*((?:S10-CS-[0-7][^*]+)|S10 (?:LOCAL_READY|REAL_PROVIDER_VERIFIED \/ MAINLINE_ABSORPTION|MAINLINE_LOCAL_READY))\*\*$/m)?.[1];
  const stagePointer = taskCenter.match(/^\*\*((?:S10-CS-[0-7][^*]+)|S10 (?:LOCAL_READY|REAL_PROVIDER_VERIFIED \/ MAINLINE_ABSORPTION|MAINLINE_LOCAL_READY))\*\*$/m)?.[1];
