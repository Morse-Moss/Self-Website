...
test('production environment contract exposes controls but no committed credentials', () => {
  ...
  for (const name of [
    ...
    'OPENAI_FALLBACK_1_API_KEY',
    'OPENAI_FALLBACK_1_BASE_URL',
    'OPENAI_FALLBACK_2_API_KEY',
    'OPENAI_FALLBACK_2_BASE_URL',
  ...
  assert.doesNotMatch(
    source,
    /^MORSE_CHAT_(?:V2|SAFE_MODE|CONTEXT_(?:PACKET_ENABLED|CANARY_))=/m,
  );
...
test('production runbooks govern disabled-first Chat v2 rollout and schema-compatible rollback', () => {
  for (const relativePath of [
    ...
    assert.match(runbook, /MORSE_CONTEXT_PACKET_DIGEST_KEY_ID/);
