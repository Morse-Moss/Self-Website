...
3. 在 `/opt/revolution/shared/secrets/context_packet_digest_key` 生成独立 32-byte Secret，owner `1001:1001`、mode `0600`，只挂载给 Web。共享环境设置 `MORSE_CONTEXT_PACKET_DIGEST_KEY_ID`、Context Packet enabled、canary percent `0`、空 UUID 白名单与空标签白名单。升级 migration `013` 并启用动态上下文后，删除旧 12k/24k 以及输入、历史、检索、attempt、输出固定上限变量。
