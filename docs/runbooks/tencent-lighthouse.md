...
7. `005` / `006` / `007` 均为 additive migration，不执行 down migration；`006` 只增加并回填非敏感邀请备注快照，`007` 只增加路由锚点、证据分类、attempt 模式与分段延迟字段。旧镜像会忽略新增列和表，确认开关已降级且 live/ready 正常后可切回上一冻结镜像，不删除迁移或数据。
