...
回退创建新 revision，引用仍可运行的旧 snapshots；前一 active revision 从被当前 revision
切离的时刻起 30 分钟内免重测，超过窗口按新目标测试规则执行。
...
scope:
  owned:
    ...
    - tests/admin-provider-integration.test.ts
    - tests/provider-config-input.test.ts
    - tests/retention-integration.test.ts
...
runtime 的数据库目标同时返回稳定 model series ID 与精确 version ID；活动历史快照只按稳定 ID
映射。上一活动版本通过独立“回退上一版”命令和 `rollbackToPrevious: true` API 意图恢复，不能把
最新模型版本 UUID 当作旧快照回退请求。
...
4. 要求先完成 `next build`，再以这些临时 env 和受控 `NODE_ENV=test` 自行启动 `next start` 子进程并固定使用 3012；不得依赖调用者预先启动的 server，也不得放宽生产 Mock 禁令；
5. 通过 local-release-smoke 专用注入让受控 transport 固定连接该 mock，生产 preflight 明确拒绝此模式；
6. 登录、创建中转/首模型、模拟 discover 失败后再手填两个模型、执行 mock test，并在桌面和移动端实际组成六线路草稿，再激活通过测试门的子集；
7. 覆盖 route editor、移动表单、密码复验层、409 conflict、delete 结果和 catalog empty，并逐层断言无横向溢出；
...
9. 断言 console/page error 为零，浏览器意外外部 origin 为零；
10. finally 中逐项清理 browser、Next、mock 与 disposable database，聚合所有失败；子进程未退出或 3012/18092 端口未释放必须令验收失败。
...
视觉脚本负责启动已构建的 `next start` server 并完成双宽证据；另以无截图的独立 `next dev`
readiness 验证 `/api/health/live`、`/admin` 和 `/admin/api`，结束后停止所有子进程，不留后台服务。
...
```powershell
git add -- app/admin/layout.tsx app/admin/page.tsx app/admin/api/page.tsx app/api/admin/_shared.ts app/api/admin/providers/routes/activate/route.ts components/admin/AdminShell.tsx components/admin/AdminShell.module.css components/admin/AdminConsole.tsx components/admin/AdminConsole.module.css components/admin/AdminApiConsole.tsx components/admin/AdminApiConsole.module.css components/admin/AdminProviderLibrary.tsx components/admin/AdminRouteEditor.tsx components/admin/AdminProviderForm.tsx components/admin/AdminReauthDialog.tsx components/admin/admin-api-client.ts components/admin/admin-client.ts lib/server/admin-provider-config.ts lib/server/ai-config-store.ts lib/server/ai-config.ts lib/server/provider-config-input.ts package.json scripts/admin-api-visual-smoke.mjs tests/admin-api-management-ui-contract.test.ts tests/admin-api-visual-smoke-contract.test.ts tests/admin-provider-api-contract.test.ts tests/admin-provider-integration.test.ts tests/provider-config-input.test.ts tests/retention-integration.test.ts tests/s10-admin-ui-contract.test.ts tests/resume-admin-ui-contract.test.ts tests/site-shell-contract.test.ts tests/routes-contract.test.ts docs/verify/admin-api docs/portfolio-blueprint.md docs/superpowers/plans/2026-07-21-admin-api-management.md docs/superpowers/specs/2026-07-21-admin-api-management-design.md
