...
- 同一代码镜像支持 web、worker、migration 和 ingest 四种显式命令；migration 不在每个 web replica 启动时自动并发执行，ingest 独立执行公开知识摄取。
...
- 增加生产镜像/命令合同、web/worker/migration/ingest 运行方式和部署 runbook。
