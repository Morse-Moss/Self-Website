...
- Admin 登录只需要 scrypt 管理员密码；连续五次失败后临时锁定。
- 管理写操作与导出检查 Origin；登录失败统一响应，数据库计数并临时锁定。
- 导出前必须再次输入管理员密码；输错密码不得注销仍有效的管理员 Session。JSON/CSV 直接返回，不在服务器落文件。
- 创建邀请码必须持有有效管理员 Session 并检查精确 Origin；邀请码由服务端生成，数据库只保存 SHA-256，明文只在创建响应中出现一次。
- CSV 对公式前缀、引号、换行和编码做防护。
- 邀请码兑换按来源指纹限流，攻击事件进入幂等 Outbox。

本轮零新增运行依赖。密码哈希与安全随机值使用 Node `crypto`。
...
- disposable pgvector 中迁移升级、重复执行、checksum 漂移、时间旅行清理、并发、补偿、登录锁定、Outbox 重放和 admin 权限矩阵全通过且零 skip。
