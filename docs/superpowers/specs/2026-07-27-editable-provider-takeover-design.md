...
该迁移是纯新增，不改写现有 Provider、活动路由或历史 attempt。本地功能迁移固定使用 `010`。后续迁移链修复已确认 `009_db_growth_indexes.sql` 归属数据库增长索引，并固定顺序为 `008 -> 009 -> 010 -> 011`；`011` 只负责已登记历史 `008` 的向前 Task Frame 修复，不改写 `008` 文件。该结论只解除本地迁移链阻断，不代表已 push、生产迁移或部署。
...
1. 在维护窗口或受控停写下按顺序应用 `009`、`010`、`011`；`011` 对 canonical `008` 为 no-op，对精确匹配的历史 `008` 执行原子向前修复，其他结构必须停止发布；
2. 同步 runtime grants 与权限验证脚本；
