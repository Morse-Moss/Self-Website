...
- 当前允许指针:M3-RAG 已通过 merge commit `d1ebd88` 吸收到本地与远端 `master`(`https://github.com/Morse-Moss/Self-Website.git`)。部署、域名与终审内容增量均不自动推进。
...
git:本地与远端 `master` 已包含 M3-RAG;`origin/codex/m3-rag-mvp` 仅保留为历史分支;**禁止自动部署**。
...
- push 仅在摩斯明确指令下执行(本轮功能分支与 `master` 同步已完成);不自动部署;依赖仅限契约列明(S3=gsap)
