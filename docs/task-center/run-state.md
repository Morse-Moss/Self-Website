...
## current_pointer
**HR_PROVIDER_TIMEOUT_RECOVERY / VERIFY / LOCAL_READY_C0CC789_DIRTY**

## next_allowed_pointer
当前生产仍为 `c449668`，`master / origin/master` 为 `c0cc789`。完整 JD turn 已确认路由、同一 Task/JD scope 与 `6 sources / 6 evidence` 均正确，失败原因为 Provider 时序误杀：primary 在内部首包 `20,008 ms` 前没有协议事件而被 `PROVIDER_FIRST_BYTE_TIMEOUT` 中止，fallback 已开始输出后又撞上共享 `80,214 ms` 总截止。用户提供的主 API 控制台证据显示同一类长回答首字 `24.52s`、总耗时 `50.27s`、约 `6,554` input / `2,675` output tokens，证明 20 秒门限会误杀本可成功的主请求。当前 owned diff 将 first-byte/protocol/model-text/provider-stage/chat-turn 调整为 `40s / 35s / 70s / 300s / 330s`，Provider total 为 `300s`；保留串行 failover、共享绝对 deadline、无 hedging、无输出 token 限制。配置测试 `31/31`、Provider/timeout 聚焦回归 `101/101`、完整套件 `1258/1258`、PostgreSQL integration `347/347`、typecheck、scoped ESLint、33-route build 与 diff check 已通过。下一步只允许完成 CRITICAL 差异复核、精确 commit/push，备份并更新生产六个超时环境值，只重建 Web/Worker；DB、Embedding、Edge、migration、grants、ingest 保持不动。发布健康通过后创建全新精确标签 `HR interview`、3 小时、1 Session 邀请，用隔离 Cookie Session 依次执行招聘入口、完整 JD 与 10 问；任一答非所问、0 evidence、编造、拒答、5xx 或 Task 切换立即停测并检查 turn 元数据。
...
- Root cause：规划器在混合能力命中时只从已有 knowledge 生成 admissions，丢弃 `evidenceClass=none` 的 Cursor；服务层也未消费 unavailable admissions，Provider 只看到正向证据而看不到必须披露的可信边界。
- Fix：正常与检索降级路径都在真实来源 admissions 之外保留 unavailable capability；服务层只提取非空 ID，Context Packet 以结构化、转义、HMAC 保护的 `<capability_evidence_boundaries>` 强制逐项披露。Cursor 不伪造成 `KnowledgeSource`，不进入 `<approved_evidence>`，也不增加输入、输出、历史、检索、模型或 Provider attempt 限制。
- Verification：planner `14/14`、Context Packet `13/13`、生产形状 PostgreSQL `1/1` 先红后绿；完整 unit `904/904`、PostgreSQL integration `344/344`、typecheck、scoped ESLint、33-route build、`git diff --check` 与敏感模式扫描通过。并行重负载曾触发一条未修改 failover 测试的 10 ms/25 ms 计时分类竞争；单用例、完整文件和串行完整 unit 均通过，本轮未扩大修改该边界。
- Review：既有 CRITICAL 独立审查代理均在返回 verdict 前被共享 Responses HTTP 502 阻断；不得伪报通过。主控分别检查质量与合规视角，当前无已知 blocker；最终效果仍以全新生产 12 轮真实 HR 链为门禁。

## HR JD 正文与证据召回修复（2026-07-29，已部署 `0c7f56a`）
...
## HR 招聘会话完整 JD 修复（2026-07-29，已部署 `e746ea4`）
...
## HR JD 能力证据补全（2026-07-29，已部署 `2440473`）
...
- Mode: `CEO / STAGED / CRITICAL / DEPLOYED`; reached: `PRODUCTION_OBSERVED / REVIEW_PASS / REAL_PROVIDER_NOT_RUN`。
...
- Release: `e5f9210` 已进入 `origin/master` 与 `origin/codex/chat-v2-release`；冻结归档 23,695,360 bytes、SHA-256 `2eca5624af1824670d988f89bc4c2a41366ed5a4523960fbb9695f82003cb122`，`/opt/revolution/current` 与 Web/Worker/Edge/DB working directory 指向该 release。
- Data/Observation: pre-migration DB 备份 SHA-256 `2f7c9fd881c74e7f2721aace21d1af933b25181ba3c45f2ddf4d1186e0863c0c`；migration 007、grants、AI/简历 runtime 权限、40 documents / 47 chunks 与二次 40/40 skip 通过。公网 health/root/works/admin 为 200，未登录受控 API 为 401，release smoke PASS，最终 2 分钟 Web/Worker/Edge/DB 错误关键词为 0。
- Recovery incident: 生产宿主 TLS 源文件已丢失，plain `compose run migration` 重建 DB 后因空证书失败；数据卷/备份完整。TLS 已持久化到 `/opt/revolution/shared/postgres/tls`，DB 基于原卷恢复 healthy 后才应用 007。升级一次性容器现要求先验健康并使用 `--no-deps`；pre-007 镜像不再是 readiness-compatible rollback。
- Boundary: 部署阶段未调用真实 Chat/Embedding/Search Provider，未创建或读取邀请码，未读取生产管理员密码、Provider key、私密简历、allowlist 值或 raw prompt/answer；历史 attempts 保持 36，active v2 Session 保持 0。
