...
S10 Provider 与对话交互修正已在本地 `master` 真实验收并随本轮精确提交。当前修正尚未 push、部署；真实博查和真实飞书仍需单独授权与凭据。
...
## Post-closeout Correction Registry
...
|---|---|---|
| `S10-E1 GPT models` | PASS | 受控兼容 User-Agent 下 `/models` HTTP 200；2026-07-17 修复时快照为 12 个且不含 `gpt-5.4-mini`，收口前已变为 17 个并重新包含 mini 与 `gpt-5.4`。目录按实时结果核对，运行选定 `gpt-5.4` |
