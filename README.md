...
- S10 数字摩斯智能客服已达到 `MAINLINE_PROVIDER_READY / CHAT_UX_LOCAL_READY`：访客三流程、自动搜索、管理后台与离线评测完成；19/19 Mock E2E、1440/390 真实浏览器、499/499 零 skip 全量测试、BGE/pgvector 语义评测和 17/17 生产构建均已通过。2026-07-17 针对中转 WAF 和易变模型目录加入显式兼容 User-Agent，并固定使用已验收的 `gpt-5.4`；随后修复默认问题只填框、等待态仍显示建议、Markdown 源码外露、来源编号不清和空 Responses 流偶发失败。访客正文现在直接显示“依据：资料标题”，底部只列本轮实际引用的具名资料；空流且尚无正文时只自动重试一次，已有部分回答时不重试。最新真实 turn `92102e75-47d5-47bc-a2da-644105d94ec3` 到达 SSE `done` 和数据库 `completed`，usage 为 2612 输入 / 73 输出 token，页面无裸 citation 或 Markdown 标记。费用因未配置单价保持未知。真实博查/飞书未验收；当前修正精确提交到本地 `master`，未 push、未部署。
