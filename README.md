...
- 管理员固定入口为 `/admin`，不出现在公开导航；管理员只使用独立密码登录，访问安全仍依赖管理 Session、精确 Origin、登录失败锁定和服务端权限校验，不依赖隐藏 URL。邀请码管理增量为 `50a7663`，密码登录简化为 `48d13b9`；两者均仅在本地隔离分支，尚未进入 `master`、push 或当前生产 release `b15be68`。
...
管理员入口不放在公开导航。开发环境直接访问 `http://localhost:3000/admin`；生产入口固定为 `https://aimorse.tech/admin`。使用独立管理员密码登录后，点击顶部“邀请码”，填写名称、有效小时数和最大会话数即可生成邀请码。系统只显示一次明文，关闭后不能恢复，必须当场复制并通过受控渠道发送。导出私有对话数据时会要求再次输入管理员密码。
