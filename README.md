...
- S10 数字摩斯智能客服已达到 `MAINLINE_PROVIDER_READY`：访客三流程、自动搜索、管理后台与离线评测完成；17/17 Mock E2E、1440/390 真实浏览器、493/493 零 skip 全量测试、BGE/pgvector 语义评测和 17/17 生产构建均已通过。2026-07-17 针对中转 WAF 和易变模型目录加入显式兼容 User-Agent，并固定使用已验收的 `gpt-5.4`；最终直连站内 Responses turn `b8b3ec78-380d-4211-9baa-9633f1847d75` 取得 5 个 RAG 来源、SSE `done` 和数据库 `completed`，usage 为 1779 输入 / 297 输出 token。费用因未配置单价保持未知。真实博查/飞书未验收；未 push、未部署。
