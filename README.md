...
- S10 数字摩斯智能客服已达到 `MAINLINE_PROVIDER_READY`：访客三流程、自动搜索、管理后台与离线评测完成；17/17 Mock E2E、1440/390 真实浏览器、493/493 零 skip 全量测试、BGE/pgvector 语义评测和 17/17 生产构建均已通过。2026-07-17 针对中转 WAF 和模型目录变化加入显式兼容 User-Agent，并切换到当前可用的 `gpt-5.4`；最终站内 Responses 全链取得 HTTP 200、5 个 RAG 来源、SSE `done` 和数据库 `completed`，usage 为 1747 输入 / 125 输出 token。费用因未配置单价保持未知。真实博查/飞书未验收；未 push、未部署。
...
部分 OpenAI-compatible 中转会拦截 SDK 默认 User-Agent。仅在模型列表用默认 SDK 请求返回 403、而同凭据的受控兼容请求返回 200 时，设置 `OPENAI_COMPAT_USER_AGENT`；值必须是单行且不超过 256 字符。模型 ID必须以中转当前 `/models` 返回为准，不能沿用已下线模型。
