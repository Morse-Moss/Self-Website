...
test('an active recruiter task keeps a catalog-shaped match question in the task frame', () => {
  const result = resolve('你有什么项目是跟我们公司相匹配的吗？', {
    audienceIntent: 'recruiter',
...
test('a deictic ownership question keeps the uniquely named adjacent project grounded', () => {
  const result = resolve('你在这个项目里具体负责什么？是团队协作，还是你主导完成的？', {
    audienceIntent: 'recruiter',
    mode: 'interviewer',
    discourseContext: completedTurn({
...
test('adjacent recruiter evaluation questions keep the exact JD frame without extracting new slots', () => {
  ...
  const questions = [
    ...
    '如果我把这个岗位的业务目标变成可执行产品方案，你会如何拆解需求、确定优先级和验收标准？请结合你做过的项目说明。',
    '你在实际项目里如何使用 Claude Code、Codex、WorkBuddy 等工具协作？哪些环节由你自己验证，如何避免 AI 生成错误？',
    '如果要维护跨境电商独立站，你会如何设计线上监控、日志、告警、数据反馈和故障处理？请区分已做过和可迁移方案。',
    '你提到快速上线。面对需求不清、资源有限和上线时间紧，你如何做最小可行版本、风险控制与反馈闭环？',
    '请具体讲一次你处理 AI Provider 超时、SSE 中断、任务失败或状态不一致的经历：怎么定位、恢复、重试和补偿？',
    '如果产品要做商品知识问答或 AI 导购，你会怎样设计 RAG/Agent 的知识来源、检索、引用和边界？',
    '你如何判断一个需求该直接执行、继续追问、需要人工确认还是拒绝？请结合内容创作 Agent 或数字摩斯说明。',
    '独立站的版本发布出现回归时，你会如何做灰度、回滚和复盘？请明确哪些是你已有证据、哪些是建议方案。',
    '你如何与设计、研发、运营协作推进跨部门项目，同时保证产品决策和交付节奏？请给出真实案例。',
    '如果让你入职后30天改进这个跨境电商产品，你会先验证哪些假设、看哪些指标、交付哪些里程碑？',
    '如何接手陌生的 AI 生成前后端代码？',
    '如何保证快速交付仍可验证、可回滚？',
    '如何在主备模型之间切换并保证可回滚？',
