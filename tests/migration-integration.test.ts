...
test('chat response reliability migration applies auditable nullable contracts', async () => {
  const database = await createDisposablePostgresDatabase();
  try {
    const result = await runMigrations(database.connectionString);
    assert.equal(result.code, 0, result.stderr);
    await withPostgresClient(database.connectionString, async (client) => {
      const columns = await client.query<{ table_name: string; column_name: string }>(
        `SELECT table_name, column_name
          ...
          WHERE table_schema = 'public'
            AND (
              (table_name = 'interaction_turns' AND column_name IN
                ('route_kind', 'route_reason_code', 'topic_kind', 'topic_ref',
                 'evidence_class', 'inherited_from_turn_id'))
              OR
              (table_name = 'interaction_provider_attempts' AND column_name IN
                ('launch_kind', 'generation_mode', 'first_protocol_event_ms',
                 'first_model_text_ms', 'first_user_visible_ms'))
              OR
              (table_name = 'chat_provider_attempts' AND column_name IN
                ('generation_mode', 'first_protocol_event_ms',
                 'first_model_text_ms', 'first_user_visible_ms'))
            )
        ...
        client.query(
          `INSERT INTO interaction_turns
            (id, access_session_id, workflow, audience_intent, question, status,
             route_kind, delete_after)
           VALUES ($1, $2, 'chat', 'general', 'route constraint', 'completed',
             'template', now() + interval '10 days')`,
          [randomUUID(), randomUUID()],
        ),
        (error: unknown) => typeof error === 'object' && error !== null
          && 'code' in error && error.code === '23514',
